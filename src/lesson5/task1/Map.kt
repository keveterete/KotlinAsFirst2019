...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    ...
    for ((name, grade) in grades) {
        if (list[grade] == null) list[grade] = mutableListOf(name)
        else list[grade]!!.add(name)
    }
    return list
}
...
fun canBuildFrom(chars: List<Char>, word: String): Boolean {
    for (char in word.indices) {
        if (!chars.contains(word[char].toLowerCase()) && !chars.contains(word[char].toUpperCase())) return false
    }
    return true
}
...
fun hasAnagrams(words: List<String>): Boolean {
    ...
    words.forEach { set.add(it.toCharArray().sorted().toString()) }
