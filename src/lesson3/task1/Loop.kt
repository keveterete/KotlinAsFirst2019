...
fun cos(x: Double, eps: Double): Double {
    ...
    var result = 0.0
    var factor = 1.0
    while ((2 * PI) < abs(xvariable))
        if (xvariable < 0) xvariable += 2 * PI else xvariable -= 2 * PI
    do {
        val a = xvariable.pow(dividpow) / factorial(dividpow.toInt()) * factor
        result += a
        dividpow += 2
        factor *= -1
    } while (abs(a) >= eps)
